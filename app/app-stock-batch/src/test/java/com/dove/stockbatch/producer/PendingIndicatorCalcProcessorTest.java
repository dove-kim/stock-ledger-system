...
import com.dove.stockdata.domain.entity.StockDataChangeId;
...
class PendingIndicatorCalcProcessorTest {
    ...
    void shouldNotPublishWhenNoPending() {
        // Given
        when(stockDataChangeRepository.findAllByCreatedAtBefore(any())).thenReturn(List.of());
        ...
        verify(stockDataChangeRepository, never()).deleteChanges(any());
    }
    ...
    void shouldNotPublishWhenChangesAreTooRecent() {
        ...
        verify(stockDataChangeRepository).findAllByCreatedAtBefore(thresholdCaptor.capture());
    ...
    void shouldGroupByStockAndFindMinDate() {
        ...
        when(stockDataChangeRepository.findAllByCreatedAtBefore(any())).thenReturn(List.of(change1, change2));
    ...
    void shouldDeleteAfterPublishing() {
        ...
        when(stockDataChangeRepository.findAllByCreatedAtBefore(any())).thenReturn(changes);

        // When
        processor.processPendingCalculations();

        // Then
        verify(stockDataChangeRepository).deleteChanges(changes);
    }
