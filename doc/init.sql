...
-- 2. STOCK 테이블 생성
-- StockId (MARKET_TYPE, CODE)는 복합 기본 키로 사용됩니다.
CREATE TABLE STOCK (
...
CREATE TABLE STOCK_DATE (
    -- LocalDateToTimestampConverter에 따라 TIMESTAMP 대신 BIGINT로 변경
    DATE BIGINT NOT NULL PRIMARY KEY COMMENT '주식 날짜 (LocalDate -> Epoch Seconds)',
    UNIQUE (DATE) -- UNIQUE 제약 조건 추가 (이미 PRIMARY KEY가 UNIQUE를 포함하지만 명시적으로)
);

-- 4. STOCK_DATA 테이블 생성
-- StockDataId (MARKET_TYPE, STOCK_CODE, TRADE_DATE)는 복합 기본 키로 사용됩니다.
CREATE TABLE STOCK_DATA (
    MARKET_TYPE VARCHAR(10) NOT NULL COMMENT '시장 타입 (KOSPI, KOSDAQ 등)',
    STOCK_CODE VARCHAR(20) NOT NULL COMMENT '주식 코드',
    -- LocalDateToTimestampConverter에 따라 TIMESTAMP 대신 BIGINT로 변경
    TRADE_DATE BIGINT NOT NULL COMMENT '거래 날짜 (LocalDate -> Epoch Seconds)',
