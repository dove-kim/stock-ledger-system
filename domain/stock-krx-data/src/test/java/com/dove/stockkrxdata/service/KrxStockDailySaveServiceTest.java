...
class KrxStockDailySaveServiceTest {
    ...
    @Test
    @DisplayName("KRX 조회 중 예외가 발생하면 그대로 전파한다")
    void saveKrxDailyStockData_serviceException_propagates() {
        // Given
        when(krxStockService.getStockListBy(eq(testKrxMarketType), eq(testDate)))
    ...
    @Test
    @DisplayName("저장 중 예외가 발생하면 나머지 항목은 처리되지 않는다")
    void saveKrxDailyStockData_saveException_stopsProcessing() {
        // Given
        List<KrxStockInfo> mockList = Arrays.asList(
                createMockKrxStockInfo("005930", "삼성전자", testDate, testKrxMarketType,
                        1000000L, new BigDecimal("70000"), new BigDecimal("71000"),
                        new BigDecimal("69500"), new BigDecimal("71500")),
                createMockKrxStockInfo("000660", "SK하이닉스", testDate, testKrxMarketType,
                        500000L, new BigDecimal("100000"), new BigDecimal("101000"),
                        new BigDecimal("99000"), new BigDecimal("102000"))
        );
        when(krxStockService.getStockListBy(eq(testKrxMarketType), eq(testDate)))
        ...
        doThrow(new RuntimeException("DB error"))
                .when(stockDataSaveService).update(
                        any(), any(), eq("005930"), any(), any(), any(), any(), any(), any());

        // When & Then
        org.junit.jupiter.api.Assertions.assertThrows(RuntimeException.class,
                () -> krxStockDailySaveService.saveKrxDailyStockData(testMarketType, testDate));

        // 첫 번째 항목에서 예외가 발생했으므로 두 번째 항목은 호출되지 않음
        verify(stockDataSaveService, never()).update(
                any(), any(), eq("000660"), any(), any(), any(), any(), any(), any());
    }
